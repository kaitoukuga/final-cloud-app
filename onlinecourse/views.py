...
def submit(request, course_id):
    ...
    # Adding the selected choice to submission table
    for item in choices:
...
def show_exam_result(request, course_id, submission_id):
    ...
    answer = len(submission_obj.choices.all().filter(is_correct=True))
    score = answer / len(submission_obj.choices.all())

    context = {}
    context['course'] = course_obj
    context['selected_ids'] = selected
    context['mark'] = score * 100
